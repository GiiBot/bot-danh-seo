...
class SeoProfilePaginator(discord.ui.View):
    ...
    def build(self, guild):
        ...
        embed = discord.Embed(
            title=f"🧬 HỒ SƠ SẸO – {member.display_name if member else self.user_id}",
            description=f"🧾 Case `{r['case']}`",
            color=CIARA_LEVEL_COLOR.get(min(len(records), 3))
        )
        ...
        embed.add_field(name="☠️ Tổng sẹo", value=str(len(records)), inline=False)
        embed.set_footer(
            text=f"{CIARA_FOOTER} • Trang {self.page+1}/{len(records)}",
            icon_url=CIARA_ICON
        )
        banner = get_ciara_banner(len(records))
        if banner:
...
class GhiSeoModal(discord.ui.Modal, title="⚔️ GHI SẸO – LORD OF CIARA"):
    ...
    async def on_submit(self, interaction):
        ...
        embed.add_field(name="🧾 Case", value=case_id)
        embed.add_field(name="👤 Thành viên", value=self.member.mention, inline=False)
...
async def topseo(interaction):
    ...
    embed = discord.Embed(title="☠️ BẢNG TỬ HÌNH – LORD OF CIARA", color=0x0F0F0F)
    for i, (uid, count) in enumerate(ranking, 1):
        m = interaction.guild.get_member(uid)
        embed.add_field(
            name=f"#{i} ☠️ {m.display_name if m else uid}",
            value=f"{count} sẹo",
            inline=False
        )
...
async def thongke(interaction):
    ...
    if not stats:
        return await interaction.response.send_message("✨ Tuần này sạch.", ephemeral=True)
